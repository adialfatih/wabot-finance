...
async function generatePieChart(nomor, tanggal, pemasukan, pengeluaran) {
  const configuration = {
    type: 'pie',
    data: {
  ...
  //fs.writeFileSync('./public/pie_today.png', buffer); // simpan file sementara
  const fileName = `./public/pie_${nomor}_${tanggal}.png`;
  const buffer = await chartJSNodeCanvas.renderToBuffer(configuration);
        ...
        if (isiLower === 'in today') {
          ...
          db.query(
            ...
            async (err, rows) => {
              ...
              let teks = `📥 *Pemasukan Hari Ini* (${today})\n\n`;
              let total = 0;
              rows.forEach((row, i) => {
                total += row.nominal;
